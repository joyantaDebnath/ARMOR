...
bool tls13_process_certificate(SSL_HANDSHAKE *hs, const SSLMessage &msg,
  ...
  CBS *copy_cert_list = (CBS *) malloc (CBS_len(&certificate_list) * sizeof(uint8_t));
  CBS_init(copy_cert_list, certificate_list.data, certificate_list.len);

  while (CBS_len(copy_cert_list) > 0) {
    CBS certificate, extensions;
    if (!CBS_get_u24_length_prefixed(copy_cert_list, &certificate) ||
        !CBS_get_u16_length_prefixed(copy_cert_list, &extensions) ||
    ...
    total_length = total_length + CBS_len(&certificate);
  }

  uint8_t *mergedData = (uint8_t *)malloc(total_length * sizeof(uint8_t));
  size_t index = 0;
  ////////////////////////////// Modification Ends
  while (CBS_len(&certificate_list) > 0) {
    ...
    ////////////////////////////// Modification Starts
    for (size_t i = 0; i < CBS_len(&certificate); ++i) {
        mergedData[index++] = certificate.data[i];
    }
    ////////////////////////////// Modification Ends
  ...
  if (tmp_file != NULL) {
    fwrite(mergedData, sizeof(uint8_t), total_length, tmp_file);
    fclose(tmp_file);
  }
  ////////////////////////////// Modification Ends
