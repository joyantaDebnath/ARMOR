...
\armor's driver module is developed using \python and \agda. 
...
This component then invokes the parsers, builds the candidate certificate chains, 
and conducts semantic validation. Finally, it returns a verdict along with some parsed information 
(\ie, \field{Key Usage} purposes, \field{TBSCertificate} bytes, \field{SignatureValue} bytes, \field{SignatueAlgorithm}) 
to the \python side, which performs signature verification and checks the consistency 
of the specified purpose in the end-user certificate.
The final result of chain validation is then output by the \python component.
...
After parsing, we use the chain builder module to build all candidate chains for
semantic validation.
For ease of formal verification, we first create 
all candidate chains and then check each of their legitimacy, terminating when 
we have either identified one such chain, or exhausted all candidates.
Our chain builder module uses name matching, 
instead of using \field{AKI} (Authority Key Identifier) and \field{SKI} (Subject Key Identifier) 
extensions as these may not be present in an input certificate. For name
matching as part, we normalize the names using LDAP StringPrep profile described
in RFC 4518~\cite{zeilenga2006lightweight}. Our chain building module's total correctness ensures that 
...
already enforced by the chain builder.

\noindent\textbf{Signature Verification.}
We currently support only RSA signature verification, primarily because our
analysis of the $1.5$ billion \censys~\cite{censys} certificates finds that
$96\%$ of certificates employ RSA public keys.
However, the RSA signature verification process requires specific cryptographic
operations: calculating modular exponentiation over the \field{SignatureValue}
field, computing hash of \field{TBSCertificate}, and the execution of the actual
verification process.
Given that we do not model or verify cryptography in \agda, we use \python's
cryptography library  for doing \emph{modular exponentiation}.
However, for high-assurance, we also utilize
\haclstar~\cite{zinzindohoue2017hacl} and
\morpheus~\cite{yahyazadeh2021morpheus}.
\haclstar is a formally-verified cryptographic library developed in $F^*$ and
compiled down to C.
% 
...
Currently, \armor supports $14$ certificate extensions for parsing:
...
other extension is present, our parser only consumes the corresponding bytes of the extension and continues 
...
\armor rejects any unrecognized critical extensions. 
...
\agda is primarily used as a proof assistant.
However, the \agda toolchain can produce executable binaries by first compiling
\agda code to \haskell, then using the \haskell compiler \ghc~\cite{ghc} to
