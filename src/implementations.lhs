...
\textbf{Driver and Input Interface.} 
The Armor's driver module is developed using both \python and \agda programming languages. The \python component is responsible for the user interface, handling inputs such as certificates in DER or PEM formats, trusted CA certificates in PEM format, and optionally, the intended purpose of the end-user certificate (like Server Authentication, Client Authentication, Code Signing, Email Protection, Time Stamping, or OCSP Signing). After receiving these inputs, the \python part forwards them to the \agda component. On the \agda side, the current time is read directly from the system. This component then invokes the necessary parsers, builds the candidate certificate chains, and conducts semantic validation as required. Finally, it returns feedback with some parsed information (\ie, \texttt{Key Usage} purposes, \texttt{TBSCertificate} bytes, \texttt{SignatureValue} bytes, \texttt{SignatueAlgorithm}) to the \python side. Subsequently, the \python side takes over to perform signature verification and check the consistency of the specified purpose in the end-user certificate. The final result of the chain validation process is determined and provided by the \python implementation. 
...
\textbf{Chain Building and String Canonicalization.} 
We use the chain builder module to build candidate chains that can potentially
be validated after parsing. Our Chain builder module locates the issuer CA certificate of the current certificate by matching the subject name of other given certificates. The algorithm to match two names are defined in Section
...
\textbf{Semantic Validation.} For semantic validation, we consider $27$ rules; the complete list is provided in Table~\ref{rules} of Appendix. The first 18 rules (R1 - R18) are applicable to individual certificates in a chain, whereas the last 9 rules (R19 - R27) are for a chain of certificates. Note that the rules from R1 to R25 are implemented in \agda while R26 (signature verification) and R27 (certificate purpose check) are enforced by the \python side of driver module. However, R24 (subject and issuer name chaining) and R25 (trust anchor check) are not explicitly enforced by the semantic validator since these checks are already enforced when candidate chains are built.
