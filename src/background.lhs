...
This section provides a brief discussion on \xfon certificate chain validation, highlighting prior works on testing open-source \xfon implementations, in order to underline the motivation of this work.
...
on the \xsno Distinguished Encoding Rules (\der)~\cite{rec2002x}. However, this \der representation of the certificate byte stream internally follows a $<T, L, V>$ structure to represent each certificate field, where $T$ denotes the
...
\textbf{c. Overall Complexities:} The \xfon certificate chain validation algorithm can be conceptually decomposed into different stages (see Figure~\ref{cert_validation}). In general, the algorithm receives two \emph{Privacy Enhanced Mail} (\pem)~\cite{balenson1993privacy} formatted certificate files and the current system time as inputs. One \pem file contains the end-user certificate and the relevant CA certificates to assist building legitimate chains, while the other \pem file contains the certificates of trusted CAs. \emph{First}, \circled{1} these \pem inputs are parsed to get each certificate in their \basesf format, and then \circled{2} the \basesf decoder is applied to each certificate to get its serialized byte stream in \der. \circled{3} Each \der certificate is then parsed using a \der parser into an intermediate representation for the next validation stages. Prior to \circled{6} enforcing the semantic checks (\eg, expiration date check, signature verification), as specified in RFC 5280~\cite{cooper2008internet}, \circled{4} string canonicalization and \circled{5} chain building are sequentially performed. While string canonicalization is a type of string conversion to ensure all the strings in a certificate are decoded in their \emph{normalized} forms~\cite{zeilenga2006lightweight}, chain building generates a candidate certification path from a trusted CA to the end-user certificate~\cite{cooper2005rfc}.
...
Similarly, converting strings to their \emph{normalized} forms is also a complex
...
them in a secure manner is nontrivial.
