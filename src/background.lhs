...
\subsubsection{Drawing Boundary} A clear separation between the parsing and semantic checks is pivotal in formally specifying the CCVL requirements. However, having a balance is also important-- too many semantic checks incorporated into the parsing process can lead to an overly complex parser while excluding all semantic checks during parsing can result in an overly lenient parser. Our strategy lies somewhere in the middle, taking inspiration from the re-engineering effort by Debath et al~\cite{debnath2021re}. Similar to that prior work, we categorize DER restrictions as part of the parsing rules, and the rest are left for semantic checks. We enforce the semantic check on DER's $<T, L, V>$ length boundness into the parsing side, contributing to a manageable parser that maintains necessary rigor without becoming overly complex.

\subsubsection{Modularity} Adopting a modular approach to implementing the X.509 CCVL can significantly mitigate some challenges. The entire process can be broken down into smaller, manageable components or modules. Each module is designed to perform a specific function, such as DER parsing, certificate chain building, string transformation, and semantic checks. Such modularization allows us to precisely specify the requirements for each module and independently establish their correctness. In addition, instead of trying to accomplish everything in a single step, this modularization allows us to undertake the validation task in multiple passes, increasing the simplicity and manageability of the overall process.

\subsubsection{Specificity} While the X.690 DER and RFC 5280 are comprehensive and detail numerous restrictions and possibilities, in reality, not all aspects of the specifications are uniformly or widely used. For example, not all the extensions specified in the standard are used in real-world certificates. In addition, RFC 5280 puts additional restrictions on certain DER rules to be used for the Internet Domain. Therefore, we aim to create a formally verified reference implementation that focuses primarily on the most commonly used fragments of the standard specifications.