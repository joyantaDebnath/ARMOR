...
% \says{joy}{Background of X.509 certificates, its importance, and the process of certificate chain validation. The need for formal verification of such a critical process. A short overview of the paper and its contributions.}
...
% \textbf{Overarching objective.} \says{joyanta}{write the problem we are trying to solve (implementation flaws -> MITM attack). we can mention the threat model here with client - server example.}
...
\textbf{Motivation and Goal.} 

\textbf{Existing Work and Limitations.}

\textbf{Technical Challenges.}

\textbf{Our Insights and Approach.}
