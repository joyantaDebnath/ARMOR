...
\begin{abstract}
      ...
      The \xfon PKI is an authentication mechanism that is widely used as a 
  ...
  is hailed as the ``\emph{most dangerous
    code in the world}'', as noncompliance with the \xfon
  standard or other vulnerabilities  can
  lead to interoperability issues or even impersonation attacks.
  Almost all existing efforts in evaluating the correctness of an \xfon implementation
  rely on
  ...
  In the words of the famous computer scientist Edsger Dijkstra: \emph{Program
    ...
    absence!}
    This sentiment is corroborated by the discovery of highly influential bugs and vulnerabilities in widely used and tested open-source \xfon implementations.
  \emph{Therefore, we set out to substantially improve this
    ...
    the \xfon certificate chain validation called \armor with formal correctness guarantees (\ie, soundness, completeness, termination, unambiguity, and non-malleability)}. \armor is developed using \agda, a dependently typed functional programming language and interactive theorem prover, and features a modular architecture to cover each stage of certificate chain validation. Finally, \armor is evaluated for its specificational accuracy and runtime overhead compared to $11$ open-source \xfon implementations, showcasing its practical effectiveness and reasonable performance in real-world applications.
