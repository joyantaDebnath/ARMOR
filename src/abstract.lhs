...
\begin{abstract}
  ...
  Almost all existing efforts in evaluating the correctness of an \xfon
  ...
  \emph{Therefore, we set out to substantially improve this
    ...
    the \xfon certificate chain validation called \armor with formal correctness guarantees (\ie, soundness, completeness, termination, unambiguity, and non-malleability)}. \armor is developed using \agda, a dependently typed functional programming language and interactive theorem prover, and features a modular architecture to cover each stage of certificate chain validation. Finally, \armor is evaluated for its specificational accuracy and runtime overhead compared to $11$ open-source libraries, showcasing its practical effectiveness and reasonable performance in real-world applications.
