...
There are several challenges for developing a formally-verified implementation for X.509 certificate validation.

\subsubsection*{a. Complexities in specifications} The \xfon specification is distributed 
...
\subsubsection*{b. Complexities in DER parsing} The internal data structure of an \xfon certificate, while described in the
...
\subsubsection*{c. Complexities in individual stages} The \xfon certificate chain validation
...
\subsection{Our Insights} We now discuss some design choices of our approach.

\subsubsection*{a. Modular decomposition} Inspired by prior work~\cite{debnath2021re, nqsb-tls}, we design and develop the X.509 certificate chain validation with modularity (based on the conceptual stages mentioned earlier). The entire process is broken down into smaller, manageable modules and each module is designed to perform a specific function, such as \der parsing, certificate chain building, string canonicalization, and semantic validation. Such modularization allows us to precisely specify the requirements for each module and independently establish their correctness with machine-checked proofs. In addition, instead of trying to accomplish everything in a single step, this modularization allows us to undertake the chain validation task in multiple passes, increasing the simplicity and manageability of the overall process.

\subsubsection*{b. Specificity} As discussed earlier, RFC 5280 comprises two main rule sets: \emph{producer rules} and \emph{consumer rules}. Our formalization specifically concentrates on the \emph{consumer rules}, which are intended for
certificate chain validation implementations. Additionally, a clear separation between the syntactic and semantic rules is pivotal in formally specifying the chain validation requirements. However, having a balance is essential --- too many semantic checks incorporated into the parsing process can lead to an overly complex parser, while excluding all semantic checks during parsing can result in an overly lenient parser. Our strategy lies somewhere in the middle, taking inspiration from the re-engineering effort by Debnath \etal~\cite{debnath2021re}. Similar to that prior work, we categorize \der restrictions as part of the parsing rules, and the rest are left for semantic validation. We enforce the semantic check on \der's $<T, L, V>$ length bound into the parsing side, contributing to a manageable parser that maintains necessary rigor without becoming overly complex. Finally, we focus primarily on the most commonly used subset of the standard specifications. While the \xsno \der and RFC 5280 are comprehensive and detail numerous restrictions and possibilities, in reality, not all aspects of the specifications are uniformly or widely used. For example, not all the extensions specified in the standard are used in real-world certificates. Thus, we did measurement studies on real certificate dataset and consulted prior work~\cite{debnath2021re} to figure out which features to support or not (details are discussed in Section 6). 

\subsubsection*{c. No verification of cryptographic operations} Verification of cryptographic
operations is out-of-scope for this work. Therefore, we do not provide any formal specification and correctness guarantee for the signature verification stage. This design choice allows us to focus on the formal verification of rest of the stages while outsourcing the computationally-intensive signature verification process to external well-known libraries.
