...
\textbf{Our objective.} \says{joy}{goal, typical process to achieve that goal, but what we did, impacts, use cases}

\textbf{}

\textbf{Detailed workflow.} \says{joy}{talk about the workflow, input output interfaces}



\section{Implementation}

\textbf{The glucode in Python} \says{joy}{how the input interface works, show the commands} 

\textbf{The glucode in Agda} \says{joy}{how the input interface works, show the commands} 

\textbf{Chain building} \says{joy}{we did name match calling string prep, can work for both ca/ca-missing, outof order, multiple valid chains, gets trusted cert from root store} 

\textbf{StringPrep} \says{joy}{challenges, manual effot, not doing for all utf8} 

\textbf{Cryptographic Operations} \says{joy}{rsa (describe the process), mod exp, hash, verification, hacl*, morpheus}