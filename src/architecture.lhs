...
We use the Agda theorem prover for formally verifying the CCVL implementation and the formally verified oracle of Morpheus for signature verification. Now, we present a brief overview of these tools.
...
\subsubsection{Oracle of Morpheus}
\label{mor}
Morpheus provides a rigorously validated oracle of the RSA $PKCS\#1-v1.5$ signature verification, the formal correctness of which has been established using the Coq proof assistant. The system accepts several parameters as input, such as a hexadecimal list of bytes representing the structure obtained after performing the modular exponentiation of the \texttt{SignatureValue} using the public exponent of the certificate issuer's RSA public key, the length of the public modulus of the RSA public key, the hash value of \texttt{TbsCertificate} in bytes, and the name of the signature hash algorithm. Upon execution, the oracle provides a boolean outcome, returning ``true'' if the signature verification passes and ``false'' if it does not.



\subsection{Overview of ARMOR}
Figure~\ref{armor} shows the high-level architecture of ARMOR, which consists of four core modules: driver, parser, string transformer, and semantic checker. As discussed in Section~\ref{sem}, the driver takes the input certificates in a single PEM file. We assume the input PEM file contains all the certificates in order. That means we rely on the sender (i.e., server) to provide the end-entity and CA certificates with a valid certification path. Therefore, we do not include the chain-builder module in our implementation to ease our verification steps. However, we formally verified the most challenging steps, such as parsing, string transformation, and semantic validation using the Agda theorem prover. Note that we execute signature verification and trust anchor check outside our verified semantic validation code. Finally, our driver program, written in Python, manages the I/O operations and directs the external calls needed to execute signature verification (based on the oracle of Morpheus) and trust anchor check. As mentioned in Section~\ref{mor}, some inputs to the Morpheus's oracle require pre-processing with cryptograpgic operations, such as modular exponentiation and hashing, for which we leverage Python's Cryptography library. 

% The reasoning behind specific design choices is explored in detail in Section~\ref{imp}.
...
\textbf{Parser Module:} The parser module we have developed includes both a Base64 decoder and a Distinguished Encoding Rules (DER) certificate parser. In our current configuration, we offer support for ten certificate extensions. These extensions have been selected based on their frequency of occurrence as indicated by previous research. The complete list of these supported extensions can be found in Table~\ref{ext_freq}. This suite of supported extensions provides comprehensive coverage for the most common scenarios encountered in certificate parsing and decoding.
...
\begin{table}[h]
  ...
  \centering
  \sffamily\tiny
  \caption{Frequencies of extensions in Censys dataset}
  \sffamily\tiny
...
\textbf{String Transformer Module:} To verify semantic constraints related to name chaining, our approach involves matching the issuer-distinguished name from one certificate with the subject name present in the issuer CA certificate. This matching algorithm is defined in Section 7.1 of RFC 5280 and necessitates all the strings to undergo a preprocessing phase using the LDAP StringPrep profile, as described in RFC 4518. However, the wide variety of languages and character sets present many cases to cover, leading to considerable complexity. Our initial implementation, which encapsulated all the transformations in a single Agda module, overwhelmed the compiler due to the sheer volume of data. As a solution, we have divided the transformations across 16 sub-modules, allowing for their sequential compilation, thereby enhancing the system's efficiency and manageability without compromising the comprehensiveness of the transformations.

\textbf{Semantic Checker Module:} 
Our implementation currently supports 28 semantic properties. Of these, 18 properties are applicable for verifying compliance within a single certificate, while the remaining ten are related to checking properties across different certificates in a chain. The specific semantic properties we cover are listed in Table~\ref{scp} and~\ref{ccp}. It is essential to note that we conduct the signature verification and trust anchor check outside the verified Agda code due to the computational complexity of these tasks and the requirements of external cryptographic libraries.

\textit{Signature Verification--} Our system currently supports only RSA signature verification, primarily because over $95\%$ of certificates employ RSA public keys, corroborated by our measurement studies on 5 billion real certificates in the Censys dataset. However, the RSA Signature verification process necessitates the application of specific cryptographic operations on the SignatureValue field, parsing the signed data's hash digest, and the execution of the actual verification process. Given that we do not model and verify cryptography in the Agda code, we utilize Python's cryptography library and Morpheus's formally verified code to perform the signature verification externally. This approach allows us to focus on leveraging Agda's strengths in formal verification of the parsing and validation logic while outsourcing the computationally-intensive cryptographic operations to established, trusted libraries in Python and Morpheus.

\textit{Trust Anchor Check--} The trust anchor check generally involves verifying if the root CA certificate is present in the trusted CA store of the verifier's system. However, in practice, this root store can also include intermediate CA certificates or even the end-entity certificate itself. This allows the validation process to proceed in reverse order, starting from the end-entity certificate and moving toward the root CA certificate until a match is found in the root CA store. Given that this process can be accomplished by directly mapping the DER bytestring of the input certificates to those in the root store, we delegate this task to our driver program as the final step in the validation process. This division of labor allows our system to focus on the complex task of certificate parsing and validation while leaving the more straightforward task of bytestring comparison to the driver program.


\textbf{Verified Agda Code to Executable Binary:} Agda is primarily a proof assistant, not commonly used to produce executable binaries directly. However, we can indirectly produce executable binaries by compiling Agda code to Haskell and then using GHC to generate an executable. This process begins with creating an Agda program, enabling IO operations through ``Agda.Builtin.IO''. Agda's ``compile'' command then compiles the Agda code into Haskell. The generated Haskell code is then compiled into an executable binary using the GHC Haskell compiler. However, the generated executable may not be as efficient as code written directly in a general-purpose language like Haskell.



\textbf{Driver Module:}
The driver module, written in Python, is a crucial intermediary that links the call to our formally verified Agda code with the input. It also manages the calls to the external processes responsible for signature verification and trust anchor checks. After these checks, the driver module collates the certificate chain validation process results to present to the verifier.



\textbf{Trusted Computing Base (TCB):} Our TCB comprises the Agda toolchain, which includes its native type-checker, compiler, and standard library. In addition, we trust the correctness of the GHC Haskell compiler to generate the executable binary. Furthermore, we assume the cryptographic operations provided by Python's Cryptography library are correct. Lastly, we operate under the assumption that the system's trust anchor-- the trusted root CA store-- is current and does not contain any certificates from compromised CAs.
